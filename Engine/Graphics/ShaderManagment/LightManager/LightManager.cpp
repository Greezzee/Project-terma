...
int LightManager::AddLightSource(const LightData &data) {
	...
	light_color[source_count] = { (float) data.color.r / 255.f,
			(float) data.color.g / 255.f, (float) data.color.b / 255.f,
			(float) data.color.a / 255.f };
	full_light_dist[source_count] = GraphicManager::ConvertViewSizeToReal(
			data.full_dist, view_id);
	any_light_dist[source_count] = GraphicManager::ConvertViewSizeToReal(
			data.any_dist, view_id);
...
bool LightManager::SetLightSource(unsigned ID, const LightData &data) {
	...
	pos[ID] = { p.x, p.y };
	light_color[ID] =
			{ (float) data.color.r / 255.f, (float) data.color.g / 255.f,
					(float) data.color.b / 255.f, (float) data.color.a / 255.f };
	full_light_dist[ID] = GraphicManager::ConvertViewSizeToReal(data.full_dist,
			view_id);
	any_light_dist[ID] = GraphicManager::ConvertViewSizeToReal(data.any_dist,
			view_id);
...
void LightManager::ApplyLight(sf::Shader *shader) {
	...
	shader->setUniformArray("full_light_dist", full_light_dist,
			source_max_count);
...
void LightManager::ApplyPixelLight(sf::Shader *shader) {
	ApplyLight(shader);
	shader->setUniform("pixel_size",
			GraphicManager::ConvertViewSizeToReal(pixel_size, view_id));
}

void LightManager::SetGlobalLight(Color &glc) {
